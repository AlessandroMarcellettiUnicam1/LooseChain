...
export async function translateDiagram(modeler,contract){
    ...
    const gasEstimation=await contract.methods.setInformation(activityList,messagges,participantList,messageAttributesList,controlFlowElementList,edgeConditionList).send({
      from: account[0], gasLimit: gasLimit, gasPrice: gasPrice})
